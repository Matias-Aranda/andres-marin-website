...
const SocialSidebar = () => {
    ...
    useEffect(() => {
      ...
      const handleScroll = () => {
        ...
        visible ? setShowClass("bg-background/95 backdrop-blur-lg shadow-[0px_5px_15px_rgba(0,0,0,.6)] border border-b-0") : setShowClass("translate-y-[100%]");
      };
  ...
  return (
    <div className={"fixed sm:bg-transparent bottom-0 sm:bottom-1/2 left-1/2 sm:left-auto border-primary/30 sm:shadow-[none] sm:backdrop-blur-none sm:border-transparent translate-y-0 sm:translate-y-1/2 -translate-x-1/2 sm:-translate-x-0 sm:right-0 transform flex flex-row sm:flex-col gap-4 p-4 z-[3] rounded-t-lg transition-all duration-400" + " " + showClass}>
        <Link target='_blank' href="https://www.instagram.com/andr3smarin8/">
            <img className='w-6 h-6' src="/instagram_logo.svg" alt="logo" />
        </Link>
        <Link target='_blank' href="https://www.youtube.com/@Andr3smarin8">
            <img className='w-6 h-6' src="/youtube_logo.svg" alt="logo" />
        </Link>
        <Link target='_blank' href="https://www.tiktok.com/@andr3smarin8">
            <img className='w-6 h-6' src="/tiktok_logo.svg" alt="logo" />
        </Link>
        <Link target='_blank' href="https://discord.gg/EDAPX2YYcw">
            <img className='w-6 h-6' src="/discord_logo.svg" alt="logo" />
        </Link>
    </div>
  )
}
